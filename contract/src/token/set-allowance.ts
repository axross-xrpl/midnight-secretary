...
// Usage: node --env-file=.env --import tsx src/token/set-allowance.ts <contractAddress> <newAllowance>
const [CONTRACT_ADDRESS, ALLOWANCE_STR] = process.argv.slice(2);
if (!CONTRACT_ADDRESS || !ALLOWANCE_STR) {
  console.error(
    "Usage: npm run set-allowance -- <contractAddress> <newAllowance>",
  );
  process.exit(1);
}
...
async function main() {
  ...
  try {
    ...
    console.log(`\n--- Calling setSendAllowance(${ALLOWANCE}) ---`);
    const result = await contract.callTx.setSendAllowance(ALLOWANCE);
    console.log(`  Confirmed at block ${result.public.blockHeight}`);
    console.log(`  Remaining send allowance is now ${ALLOWANCE}`);
  } finally {
    await facade.stop();
  }
}
