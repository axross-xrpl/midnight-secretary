...
// Usage: node --env-file=.env --import tsx src/shielded-token/get-state.ts <contractAddress>
// Prints one JSON object as the LAST line of stdout -- see
// src/token/get-state.ts for why (spawned by /dev/contracts's server-side
// faucet routes rather than imported into their process).
const [CONTRACT_ADDRESS] = process.argv.slice(2);
if (!CONTRACT_ADDRESS) {
  console.error("Usage: npm run get-shielded-token-state -- <contractAddress>");
  process.exit(1);
}
...
async function main() {
  ...
  try {
    ...
    console.log(
      JSON.stringify({ mintCount, initialized, mintAllowanceRemaining }),
    );
  } finally {
    await facade.stop();
  }
