...
import { witnesses, createAgeVerificationPrivateState } from "./witnesses.js";
...
async function main() {
  ...
  try {
    ...
    // No `args`: the contract declares no constructor, and the options type
    // drops the property entirely in that case. The registrations Map starts
    // empty, so the deployer holds no privileged role -- anyone can register
    // against this deployment. The placeholder DOB below is never committed;
    // register.ts writes the real private state.
    console.log("Deploying contract (this may take a minute)...");
    const deployed = await deployContract(providers, {
      compiledContract,
      privateStateId: "age-verification-private-state",
      initialPrivateState: createAgeVerificationPrivateState(DEPLOYER_SEED, 0n),
    });
    ...
    console.log("\nNext: npm run register-age -- <contractAddress> <YYYYMMDD>");
  } finally {
    await facade.stop();
  }
}
