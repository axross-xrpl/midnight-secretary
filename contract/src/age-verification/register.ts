...
// Usage: node --env-file=.env --import tsx src/age-verification/register.ts
//          <contractAddress> <dateOfBirthYYYYMMDD> [seedHex]
// seedHex defaults to USER_SEED, falling back to DEPLOYER_SEED -- give each
// test registrant a different seed to exercise the multi-registrant path
// (registrations is keyed by identity, one entry per distinct seed).
const [CONTRACT_ADDRESS, DOB_STR, SEED_OVERRIDE] = process.argv.slice(2);
if (!CONTRACT_ADDRESS || !DOB_STR) {
  console.error(
    "Usage: npm run register-age -- <contractAddress> <dateOfBirthYYYYMMDD> [seedHex]",
  );
  process.exit(1);
}
...
async function main() {
  ...
  try {
    ...
    const providers = {
      ...
      // Keyed by this registrant's own identity secret -- see deploy.ts's
      // comment on why this can't be a fixed "deployer" accountId here.
    ...
    // Fails with "Identity already registered" if this seed has already
    // registered against this deployment.
    console.log(`\n--- Calling register() for DOB ${DATE_OF_BIRTH} ---`);
